...
import { CloudSun, AlertTriangle, Package, Search } from 'lucide-react';
...
export default function Index() {
  ...
  const shelterPoints = filteredPoints.filter(
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-primary text-primary-foreground shadow-lg">
        <div className="max-w-2xl mx-auto px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div
            ...
            className="bg-card border border-primary/30 rounded-xl px-4 py-3 shadow-sm cursor-pointer w-full sm:w-auto"
          >
        ...
        <div className="max-w-2xl mx-auto px-4 py-3 flex gap-3 items-start">
          ...
          <div className="text-xs sm:text-sm font-medium">
            ...
            <p className="opacity-90 mt-1">
              Sempre confirme a autenticidade dos pontos antes de doar.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-destructive/10 border-b border-destructive/20">
        <div className="max-w-2xl mx-auto px-4 py-3 flex gap-2 flex-col sm:flex-row">
          ...
          <button
            ...
            className="flex-1 bg-primary text-primary-foreground text-sm font-bold py-2.5 px-3 rounded-xl"
          >
        ...
        <div className="flex gap-2 bg-muted p-1 rounded-xl">
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-semibold ${
              tab === 'points' ? 'bg-card shadow' : 'text-muted-foreground'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-semibold ${
              tab === 'shelters' ? 'bg-card shadow' : 'text-muted-foreground'
            }`}
          >
            🏠 Abrigos
          </button>
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-semibold ${
              tab === 'alerts' ? 'bg-card shadow' : 'text-muted-foreground'
            }`}
          >
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 pb-20">
          ...
          <>
            <div className="relative mb-4">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 border border-input rounded-xl text-sm bg-card"
              />
            </div>
        ...
        {tab === 'shelters' && (
          <div className="space-y-3">
            {shelterPoints.length === 0 && (
              <div className="text-sm text-muted-foreground text-center py-6">
                Nenhum abrigo encontrado.
              </div>
            )}
            {shelterPoints.map(point => (
              <CollectionPointCard key={point.id} point={point} />
            ))}
          </div>
        )}
